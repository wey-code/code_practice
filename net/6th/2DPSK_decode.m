...
figure;
subplot(5,1,1);plot(x,psk); ylim([-1.2 1.2]);title('已调波')%调制波 
subplot(5,1,2);plot(x,psk2); ylim([-1.2 1.2]);title('延时后波形')%接收载波
subplot(5,1,3);plot(x,sov1); ylim([-1.2 1.2]);title('相干解调后波形')%
subplot(5,1,4);plot(x,sov2); title('低通滤波后波形')%
subplot(5,1,5);plot(x,sov3); ylim([-0.5 1.5]);title('经采样判决恢复出绝对码（0-1码）')%
